...
allowed-tools: Read, Write, Edit, Glob, Grep, AskUserQuestion, Task, EnterPlanMode, TaskCreate, TaskUpdate, TaskList
---
...
```json
{
  "questions": [{
    ...
    "header": "Next step",
    "options": [
      {"label": "Discuss plan", "description": "Review and refine the plan together in chat"},
      {"label": "Auto review", "description": "Launch AI plan-review agent for automated analysis"},
      {"label": "Start implementation", "description": "Begin with task 1"},
      {"label": "Done", "description": "Plan is ready, no further action"}
    ],
    "multiSelect": false
  }]
}
...
- **Discuss plan**: read the plan file and present a summary of each task. ask the user one at a time using AskUserQuestion:
  1. "any changes to [Task N]?" with options like "Looks good", "Modify scope", "Remove task", "Split into smaller tasks"
  2. apply requested changes to the plan file via Edit
  3. after reviewing all tasks, ask "anything else to add or change?" as a final pass
  4. when the user is satisfied, ask again with the remaining options (minus "Discuss plan")
...
- **Start implementation**: begin with task 1
- **Done**: plan is finalized, stop
